...
int main(){
	...
	// Vars Code
	float multasValor[3] = {130, 195, 855}; // (20% = R$ 130, 50% = R$ 195)
	float velocidade = 0;
	float limite = 80;
	float multa = 0; 
	char placa[8];
	...
	if(multado){
		...
		printf("%s\nMotorista multado em: %.2fR$\nUltrapassou o limite de velocidade em (%0.f%%)\n%s\n",linha,multa,ultrapassou,linha);
	} else {
		printf("%s\nO motorista está dentro dos limites de velocidade! (%.2fKm/h)\n%s\n",linha,velocidade,linha);
	}
	system("PAUSE");
}
